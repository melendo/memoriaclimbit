...
\begin{enumerate}
  ...
  \item Exporta los controladores para ser utilizados por los routers al recibir las peticiones HTTP correspondientes.
\end{enumerate}
...
A través del middleware \texttt{Multer} se reciben los archivos y se les pasa una primera validación de MIME-type y tamaño máximo como filtro antes de proceder a una evaluación más exhaustiva. Una vez superada esa primera validación, a través de la librería \texttt{file-type} se obtiene el tipo real del archivo leyendo sus \textit{magic bytes} para prevenir posibles brechas de seguridad con los archivos recibidos. 

Si se superan las validaciones, se convierten al formato WebP utilizando \texttt{FFmpeg}, garantizando una comprensión eficiente que permite reducir su tamaño sin perdida de calidad apreciable. Esto se traduce en una mejora de rendimiento a la hora de servir las imágenes y un menor consumo de espacio en disco.
